...
namespace Dal
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// <param name="tableName">DataSet结果中的表名</param>
        /// <returns>DataSet</returns>
        public static DataSet RunProcedure2(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                DataSet dataSet = new DataSet();
        ...
        private static SqlConnection KeepConnection = new SqlConnection(connectionString);
        /// <summary>
        /// 执行SQL语句，返回影响的记录数(保持连接)
        /// </summary>
        /// <param name="SQLString">SQL语句</param>
        /// <returns>影响的记录数</returns>
        public static int KeepExecuteSql(string SQLString)
        {

                using (SqlCommand cmd = new SqlCommand(SQLString, KeepConnection))
                {
                    try
                    {
                        if (KeepConnection.State != ConnectionState.Open)
                          KeepConnection.Open();
                        int rows = cmd.ExecuteNonQuery();
                        return rows;
                    }
        ...
        /// <summary>
        /// 执行查询语句，返回DataSet(保持连接)
        /// </summary>
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataSet KeepQuery(string SQLString, int Times)
        {
            {
                DataSet ds = new DataSet();
                try
                {
                    if (KeepConnection.State != ConnectionState.Open)
                      KeepConnection.Open();
                    SqlDataAdapter command = new SqlDataAdapter(SQLString, KeepConnection);
                    command.SelectCommand.CommandTimeout = Times;
                    command.Fill(ds, "ds");
                }
                catch (System.Data.SqlClient.SqlException ex)
